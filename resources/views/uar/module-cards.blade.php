...
@section('title', ($currentApp->name ?? 'UAR SAP') . ' Modules')
...
<div class="app-content-wrapper">
    ...
    <main class="flex-grow-1 page-content px-4">
        ...
        @php
            $totalSessions = $modules->sum('session_count');
            ...
            <div class="directory-header">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <span style="font-size:.75rem;background:#eef4ff;color:#0b2e6d;padding:.2rem .6rem;border-radius:20px;font-weight:700;">
                        {{ $totalSessions }} Total {{ Str::plural('Session', $totalSessions) }}
                    </span>
                    @endif
                </div>
                ...
                <table class="table table-directory mb-0" id="modulesTable">
                    ...
                    <tbody>
                        @foreach($modules as $mod)
                            @php
                                $moduleUrl = route('uar.module.sessions', ['app' => $currentApp->slug, 'module' => $mod->code]);
                            @endphp
                            <tr class="module-row" data-code="{{ strtolower($mod->code) }}" data-name="{{ strtolower($mod->name) }}">
                                ...
                                <td>
                                    <span style="font-size:.82rem;color:#64748b;line-height:1.4;">
                                        {{ $mod->description ?: ('Review user authorizations and access audit for ' . $mod->name . '.') }}
                                    </span>
                                </td>
                                <td>
                                    <span style="font-weight:700;color:{{ ($mod->session_count ?? 0) > 0 ? '#0b2e6d' : '#94a3b8' }};">
                                        {{ $mod->session_count ?? 0 }}
                                    </span>
                                </td>
                                <td style="text-align:right;">
                                    <div class="d-inline-flex align-items-center gap-2">
                                        ...
                                        @if((Auth::user()->isAdmin() || Auth::user()->isPicAo()) && !in_array($mod->code, ['FM', 'PS', 'FI', 'CO', 'HR', 'MM', 'SD', 'PM']))
                                        <form method="POST" action="{{ route('uar.module.destroy', $mod->id) }}" onsubmit="return confirm('Are you sure you want to delete module &quot;{{ addslashes($mod->name) }}&quot;?');" style="margin:0;">
                                            @csrf
                                            @method('DELETE')
...
<div class="modal fade" id="modalAddModule" tabindex="-1" aria-labelledby="modalAddModuleLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15);overflow:hidden;">
            <div class="modal-header border-0 pb-0" style="padding:1.5rem 1.5rem 0.5rem;">
                <div class="d-flex align-items-center gap-3">
                    <div style="width:40px;height:40px;background:#eef4ff;border-radius:10px;display:flex;align-items:center;justify-content:center;color:#0b2e6d;flex-shrink:0;">
                        <i class="bi bi-grid-plus-fill fs-5"></i>
                    </div>
                    <div>
                        <h5 class="modal-title fw-bold" id="modalAddModuleLabel" style="color:#0b2e6d;font-size:1.15rem;margin:0;">
                            Add New Module
                        </h5>
                        <p class="text-muted small mb-0" style="font-size:.78rem;">
                            Register a new module for {{ $appName }}
                        </p>
                    </div>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('uar.module.store', ['app' => $currentApp->slug]) }}" method="POST">
                @csrf
                <div class="modal-body" style="padding:1.25rem 1.5rem 1.5rem;">
                    <div class="mb-3">
                        ...
                        <input type="text" name="code" class="form-control rounded-3" placeholder="e.g. CRM, QM, WM" maxlength="50" style="font-weight:700;text-transform:uppercase;" required autofocus>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control rounded-3" placeholder="e.g. Quality Management" maxlength="255" required>
                    </div>
                    <div>
                        <label class="form-label text-muted text-uppercase fw-bold" style="font-size:.7rem;letter-spacing:.5px;">Scope / Description</label>
                        <textarea name="description" class="form-control rounded-3" rows="3" placeholder="Brief explanation of module access authorizations..."></textarea>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0" style="padding:0 1.5rem 1.5rem;">
                    ...
                    <button type="submit" class="btn btn-primary rounded-3 px-4 fw-bold shadow-sm" style="background:#0b2e6d;border-color:#0b2e6d;">
                        <i class="bi bi-plus-circle me-1"></i> Save Module
                    </button>
                </div>
            </form>
        </div>
