...
<div class="modal fade" id="uploadUarModal" tabindex="-1" aria-labelledby="uploadUarModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <form method="POST" action="{{ route('uar.import') }}" enctype="multipart/form-data" id="uarImportForm">
                @csrf
                <div class="modal-header border-0 pb-0 pt-4 px-4">
                    <div class="d-flex align-items-center gap-3">
                        <div style="width:42px;height:42px;background:#e0edff;border-radius:12px;display:flex;align-items:center;justify-content:center;color:#0B2E6D;flex-shrink:0;">
                            <i class="bi bi-plus-circle-fill fs-5"></i>
                        </div>
                        <div>
                            <h5 class="modal-title fw-bold text-dark mb-0" id="uploadUarModalLabel" style="font-size:1.15rem;">
                                Upload UAR Excel File
                            </h5>
                            <p class="text-muted small mb-0 mt-0.5" style="font-size:.78rem;">The system will parse the file and run automated review evaluation (Review by System).</p>
                        </div>
                    </div>
                ...
                <div class="modal-body p-4">
                    {{-- File Dropzone --}}
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-dark">UAR Excel File (.xlsx / .xls) <span class="text-danger">*</span></label>
                        <div class="border border-2 border-dashed rounded-4 p-4 text-center" 
                            ...
                            <div class="fw-semibold text-dark mb-1" id="fileLabel">Choose a file or drag & drop here</div>
                            <div class="text-muted small">Supports standard template formats (e.g. <code>Hasil UAR Modul FM.xlsx</code>)</div>
                            <input type="file" name="excel_file" id="excelFileInput" class="d-none" accept=".xlsx, .xls" required
                    ...
                    {{-- Period --}}
                    <div class="mb-2">
                        <label class="form-label small fw-bold text-dark">Review Period</label>
                ...
                <div class="modal-footer border-0 pt-0 pb-4 px-4">
                    ...
                    <button type="submit" class="btn btn-primary rounded-3 px-4 fw-bold shadow-sm" id="btnSubmitImport">
                        Submit
                    </button>
                </div>
            </form>
...
function updateFileName(input) {
    if (input.files && input.files[0]) {
        document.getElementById('fileLabel').innerHTML = '<span class="text-primary fw-bold">' + input.files[0].name + '</span> (' + (input.files[0].size / 1024).toFixed(1) + ' KB)';
    }
}

document.getElementById('uarImportForm').addEventListener('submit', function() {
