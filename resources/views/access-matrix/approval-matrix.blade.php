...
<div class="app-content-wrapper">
    ...
    <main class="flex-grow-1 page-content px-4">
            ...
            ['label' => $currentApp->name ?? 'UAM SAP', 'url' => route('access-matrix.approval.app', ['app' => $currentApp->slug ?? 'sap'])],
            ...
            <div style="background:linear-gradient(135deg,#071f4d 0%,#0B2E6D 50%,#1e3a8a 100%);border-radius:18px;padding:1.4rem 2rem;position:relative;overflow:hidden;box-shadow:0 8px 20px -4px rgba(11,46,109,.2);">
                ...
                <div class="position-relative" style="z-index:1;">
                    ...
                    <h1 style="color:#fff;font-size:1.6rem;font-weight:800;margin-bottom:0;line-height:1.2;letter-spacing:-.4px;display:flex;align-items:center;gap:.65rem;flex-wrap:wrap;">
                        Approval Access Matrix <span style="font-size:1.1rem;font-weight:800;color:var(--secondary);background:#e0edff;border-radius:10px;padding:.2rem .75rem;display:inline-flex;letter-spacing:0.5px;box-shadow:0 2px 8px rgba(0,0,0,0.12);">{{ strtoupper($currentModule) }}</span>
                    </h1>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 animate-in animate-in-delay-2" style="gap:1rem;flex-wrap:wrap;">
            <form method="GET" action="{{ route('access-matrix.approval.module.list', ['app' => $currentApp->slug ?? 'sap', 'module' => $currentModule]) }}" id="filterForm"
                ...
                @if($filterYear || $filterPeriod || $search)
                    <a href="{{ route('access-matrix.approval.module.list', ['app' => $currentApp->slug ?? 'sap', 'module' => $currentModule]) }}"
